...
#define TRANSMITTER_VERSION (102)
...
void setup()
{
    ...
    Serial.print("Transmitter Start, version:");
    Serial.println(TRANSMITTER_VERSION);
...
void Wait_Until_GDO0_Is_High(void)
{
    while (!GDO0_State)
    {
...
void Wait_Until_GDO0_Is_Low(void)
{
    while (GDO0_State)
    {
       GDO0_State = digitalRead(GDO0_PIN);
...
void TxData_RF(void)
{
    ...
    SendCommandStrobe(CC2500_IDLE);
    // Flush TX FIFO
    SendCommandStrobe(CC2500_FTX);
    ...
    Serial.println();
    // STX: enable TX
    SendCommandStrobe(CC2500_TX);

    Serial.println("Wait for GDO0 to be set (sync transmitted)");
    Wait_Until_GDO0_Is_High();

    Serial.println("Wait for GDO0 to be cleared (end of packet)");
    Wait_Until_GDO0_Is_Low();
}
...
char SendCommandStrobe(char strobe)
{
    digitalWrite(SS,LOW);
